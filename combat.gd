...
func resolve_combat_round(actor_a: Node2D, actor_b: Node2D) -> void:
	...
	var a_sheet: CharacterSheet = actor_a.charactersheet if actor_a.has("charactersheet") else null
	var b_sheet: CharacterSheet = actor_b.charactersheet if actor_b.has("charactersheet") else null
